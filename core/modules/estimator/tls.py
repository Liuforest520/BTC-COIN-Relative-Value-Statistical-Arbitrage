...
@register_estimator("tls")
class TLSEstimator:
    """Estimate the price relation by equal-error orthogonal regression."""
    ...
    def update(self, state, x_close, y_close, bar_index, para=None):
        ...
        if state.spread_beta is None:
            ...
            output.ready = False
            output.reason = "tls initialized; first tradable bar is next bar" if fitted else "tls initialization failed"
            if self.collect_diagnostics:
    ...
    def _fit_model(self, state, bar_index):
        ...
        if x_arr is None:
            return False
        try:
            fitted_alpha, beta = self._tls(x_arr, y_arr)
    ...
    @staticmethod
    def _tls(x_arr, y_arr):
        x_mean, y_mean = float(np.mean(x_arr)), float(np.mean(y_arr))
        s_xx = float(np.var(x_arr, ddof=1))
        s_yy = float(np.var(y_arr, ddof=1))
        s_xy = float(np.cov(x_arr, y_arr, ddof=1)[0, 1])
        if s_xx <= 1e-12 or abs(s_xy) <= 1e-12:
            raise ValueError("TLS requires non-zero X variance and covariance")
        beta = ((s_yy - s_xx) + np.sqrt((s_yy - s_xx) ** 2 + 4.0 * s_xy ** 2)) / (2.0 * s_xy)
        alpha = y_mean - float(beta) * x_mean
