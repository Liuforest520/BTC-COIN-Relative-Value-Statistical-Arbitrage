"""Scheduled rolling-window exponentially weighted least-squares estimator."""
...
@register_estimator("ewls")
class EWLSEstimator:
    """Estimate beta with weights defined by an observation half-life."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ewls_half_life_bars = float(ewls_half_life_bars)
        if self.ewls_half_life_bars <= 0:
            raise ValueError("ewls_half_life_bars must be positive")
    ...
    def update(self, state, x_close, y_close, bar_index, para=None):
        ...
        if state.spread_beta is None:
            ...
            output.ready = False
            output.reason = "ewls initialized; first tradable bar is next bar" if fitted else "ewls initialization failed"
            if self.collect_diagnostics:
    ...
    def _fit_model(self, state, bar_index):
        ...
        if x_arr is None:
            return False
        try:
            alpha, beta = self._ewls(x_arr, y_arr)
    ...
    def _weights(self, sample_count):
        ages = np.arange(sample_count - 1, -1, -1, dtype=float)
        return np.power(0.5, ages / self.ewls_half_life_bars)

    def _ewls(self, x_arr, y_arr):
        weights = self._weights(len(x_arr))
        weight_sum = float(np.sum(weights))
        x_mean = float(np.dot(weights, x_arr) / weight_sum)
        y_mean = float(np.dot(weights, y_arr) / weight_sum)
        x_centered, y_centered = x_arr - x_mean, y_arr - y_mean
        denominator = float(np.dot(weights, x_centered * x_centered))
        if denominator <= 1e-12:
            raise ValueError("EWLS requires non-zero weighted X variance")
    ...
    def _output(self, state, bar_index, ready, latest_spread, has_position):
        ...
        if self.collect_diagnostics:
            para = {"estimator": {
                ...
                "ewls_half_life_bars": self.ewls_half_life_bars,
