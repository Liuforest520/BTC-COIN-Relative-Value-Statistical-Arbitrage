"""Scheduled rolling-window Huber robust-regression estimator."""
...
@register_estimator("huber")
class HuberEstimator:
    """Estimate beta by Huber iteratively reweighted least squares."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.huber_iterations = max(1, int(huber_iterations))
    ...
    def update(self, state, x_close, y_close, bar_index, para=None):
        ...
        if state.spread_beta is None:
            ...
            output.ready = False
            output.reason = "huber initialized; first tradable bar is next bar" if fitted else "huber initialization failed"
            if self.collect_diagnostics:
    ...
    def _fit_model(self, state, bar_index):
        ...
        if x_arr is None:
            return False
        try:
            fitted_alpha, beta = self._huber(x_arr, y_arr)
        except (LinAlgError, ValueError):
            return False
        residuals = y_arr - (fitted_alpha + beta * x_arr)
        residuals = residuals[np.isfinite(residuals)]
        if len(residuals) < 2:
            return False
        self._save_fit(state, fitted_alpha, beta, residuals, bar_index)
        return True

    def _huber(self, x_arr, y_arr):
        design = np.column_stack([np.ones(len(x_arr)), x_arr])
        coefficients = np.linalg.lstsq(design, y_arr, rcond=None)[0]
        for iteration in range(1, self.huber_iterations + 1):
            residuals = y_arr - design @ coefficients
            ...
            outliers = standardized > self.huber_delta
            weights[outliers] = self.huber_delta / standardized[outliers]
            root_weights = np.sqrt(weights)
            new_coefficients = np.linalg.lstsq(
                design * root_weights[:, None], y_arr * root_weights, rcond=None
            )[0]
