"""Scheduled age-weighted rolling WLS estimator."""
...
@register_estimator("age_weighted_wls")
class AgeWeightedWLSEstimator:
    """Use lower weights for recent observations and higher weights for old ones."""

    method_name = "age_weighted_wls"
    PROFILE_WEIGHTS = {
        "mild": (0.75, 0.85, 0.95, 1.00),
        "medium": (0.50, 0.70, 0.85, 1.00),
        "strong": (0.25, 0.50, 0.75, 1.00),
    }
    def __init__(
        ...
        age_wls_profile="medium", age_wls_weights=None,
        age_bucket_end_bars=None, age_bucket_end_fractions=None,
        age_bucket_weights=None, **kwargs,
    ):
        ...
        self.age_wls_profile = str(age_wls_profile).lower()
        if age_bucket_weights is not None and age_wls_weights is not None:
            if tuple(age_bucket_weights) != tuple(age_wls_weights):
                raise ValueError("age_bucket_weights and age_wls_weights disagree")
        selected_weights = age_bucket_weights if age_bucket_weights is not None else age_wls_weights
        if selected_weights is None:
            if self.age_wls_profile not in self.PROFILE_WEIGHTS:
                raise ValueError("age_wls_profile must be mild, medium, or strong")
            selected_weights = self.PROFILE_WEIGHTS[self.age_wls_profile]
        if age_bucket_end_bars is not None and age_bucket_end_fractions is not None:
            raise ValueError(
                "configure age_bucket_end_bars or age_bucket_end_fractions, not both"
            )
        if age_bucket_end_bars is not None:
            boundaries = tuple(int(value) for value in age_bucket_end_bars)
        elif age_bucket_end_fractions is not None:
            boundaries = tuple(
                int(round(float(value) * self.model_lookback_bars))
                for value in age_bucket_end_fractions
            )
        else:
            raise ValueError("age_bucket_end_bars is required for age_weighted_wls")
        weights = tuple(float(value) for value in selected_weights)
        if not boundaries or len(boundaries) != len(weights):
            raise ValueError(
                "age_bucket_end_bars and age_bucket_weights must have equal non-zero length"
            )
        if any(value <= 0 for value in boundaries):
            raise ValueError("age bucket end bars must be positive")
        if any(left >= right for left, right in zip(boundaries, boundaries[1:])):
            raise ValueError("age bucket end bars must be strictly increasing")
        if boundaries[-1] != self.model_lookback_bars:
            raise ValueError("the final age bucket end bar must equal model_lookback_bars")
        if any(not isfinite(value) or value <= 0 for value in weights):
            raise ValueError("age bucket weights must be finite and positive")
        self.age_bucket_end_bars = boundaries
        self.age_bucket_end_fractions = tuple(
            value / self.model_lookback_bars for value in boundaries
        )
        self.age_bucket_weights = weights
        # Backward-compatible attribute used by existing reports and callers.
        self.age_wls_weights = weights
    ...
    def update(self, state, x_close, y_close, bar_index, para=None):
        ...
        if state.spread_beta is None:
            ...
            output.ready = False
            output.reason = (
                "age_weighted_wls initialized; first tradable bar is next bar"
                if fitted else "age_weighted_wls initialization failed"
            )
            if getattr(self, "collect_diagnostics", True):
    ...
    def _fit_model(self, state, bar_index):
        ...
        if x_arr is None:
            return False
        try:
            alpha, beta = self._age_wls(x_arr, y_arr)
    ...
    def _age_weights(self, sample_count):
        sample_count = int(sample_count)
        if sample_count <= 0:
            raise ValueError("age-weighted WLS requires at least one observation")
        expected_sample_count = self._model_sample_count()
        if sample_count != expected_sample_count:
            raise ValueError(f"sample count must equal {expected_sample_count}")
        age_step = (
            self.return_interval_bars
            if self.regression_method in {"log_return", "log_returns"}
            else 1
        )
        ages = np.arange(sample_count, 0, -1, dtype=int) * age_step
        bucket_indices = np.searchsorted(
            np.asarray(self.age_bucket_end_bars), ages, side="left"
        )
        return np.asarray(self.age_bucket_weights, dtype=float)[bucket_indices]

    def _age_wls(self, x_arr, y_arr):
        weights = self._age_weights(len(x_arr))
        ...
        if denominator <= 1e-12:
            raise ValueError("age-weighted WLS requires non-zero weighted X variance")
    ...
    def _output(self, state, bar_index, ready, spread, has_position):
        ...
        if getattr(self, "collect_diagnostics", True):
            para = {"estimator": {
                ...
                "age_wls_profile": self.age_wls_profile,
                "age_bucket_end_bars": list(self.age_bucket_end_bars),
                "age_bucket_end_fractions": list(self.age_bucket_end_fractions),
                "age_bucket_weights": list(self.age_bucket_weights),
